...
class SfcIntegrationLog(Document):
	def validate(self):
		self._set_title()
	...
	def clear_old_logs(days=90):
		table = frappe.qb.DocType("Sfc Integration Log")
		frappe.db.delete(
...
def _retry_job(job: str):
	...
	doc = frappe.get_doc("Sfc Integration Log", job)
...
def create_log(
	...
	if make_new:
		log = frappe.get_doc({"doctype": "Sfc Integration Log", "integration": cstr(module_def)})
		log.insert(ignore_permissions=True)
	else:
		log = frappe.get_doc("Sfc Integration Log", frappe.flags.request_id)
...
def _retry_job(job: str):
	...
	doc = frappe.get_doc("Sfc Integration Log", job)
	if not doc.method.startswith("sfc_integrations.") or doc.status != "Error":
		return

	# doc.db_set("status", "Success", update_modified=False)
	doc.db_set("traceback", "", update_modified=False)
